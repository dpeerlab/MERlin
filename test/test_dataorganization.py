...
def test_dataorganization_get_channels(simple_merfish_data):
    assert np.array_equal(
        simple_merfish_data.get_data_organization().get_data_channels(), np.arange(18)
    )


def test_dataorganization_get_channel_name(simple_merfish_data):
    for i in range(16):
        assert simple_merfish_data.get_data_organization().get_data_channel_name(
            i
        ) == "bit" + str(i + 1)

    assert (
        simple_merfish_data.get_data_organization().get_data_channel_name(16) == "DAPI"
    )
    assert (
        simple_merfish_data.get_data_organization().get_data_channel_name(17) == "polyT"
    )
...
def test_dataorganization_get_channel_index(simple_merfish_data):
    for i in range(16):
        assert (
            simple_merfish_data.get_data_organization().get_data_channel_index(
                "bit" + str(i + 1)
            )
            == i
        )
    ...
    assert (
        simple_merfish_data.get_data_organization().get_data_channel_index("DAPI") == 16
    )
    assert (
        simple_merfish_data.get_data_organization().get_data_channel_index("polyT")
        == 17
    )
...
def test_dataorganization_get_fovs(simple_merfish_data):
    assert np.array_equal(
        simple_merfish_data.get_data_organization().get_fovs(), np.arange(2)
    )


def test_dataorganization_get_z_positions(simple_merfish_data):
    assert np.array_equal(
        simple_merfish_data.get_data_organization().get_z_positions(), np.array([0])
    )
...
def test_dataorganization_get_fiducial_information(simple_merfish_data):
    ...
    assert os.path.normpath(data.get_fiducial_filename(1, 1)) == os.path.normpath(
        os.path.abspath("test_data/merfish_test/test_1_0.tif")
    )
...
def test_dataorganization_get_image_information(simple_merfish_data):
    ...
    assert os.path.normpath(data.get_image_filename(1, 1)) == os.path.normpath(
        os.path.abspath("test_data/merfish_test/test_1_0.tif")
    )
...
def test_dataorganization_get_sequential_rounds(simple_merfish_data):
    ...
    sequentialRounds, sequentialChannels = dataOrganization.get_sequential_rounds()
