...
def test_write_and_read_one_fov(barcode_db):
    ...
    assert np.isclose(
        barcodesToWrite.values.astype(np.float), readBarcodes.values.astype(np.float)
    ).all()
...
def test_multiple_write_one_fov(barcode_db):
    ...
    assert np.isclose(
        readBarcodes.values.astype(np.float), combinedBarcodes.values.astype(np.float)
    ).all()
    barcode_db.empty_database(fov=0)
    assert len(barcode_db.get_barcodes()) == 0
...
def test_read_select_columns(barcode_db_with_barcodes):
    ...
    assert np.isclose(
        ...
        readBarcodes[["mean_intensity", "x", "intensity_0"]].values,
    ).all()
...
def test_read_filtered_barcodes(barcode_db_with_barcodes):
    ...
    for area in range(0, 11, 2):
        for intensity in np.arange(0, 20, 5.1):
            readBarcodes = barcode_db_with_barcodes[0].get_filtered_barcodes(
                area, intensity
            )
            ...
            if len(readBarcodes) > 0:
                ...
                assert np.isclose(
                    readBarcodes.values, selectBarcodes[readBarcodes.columns].values
                ).all()
...
def test_get_barcode_intensities_with_area(barcode_db_with_barcodes):
    ...
    for area in range(11):
        readIntensities = barcode_db_with_barcodes[
            0
...
def test_get_barcode_values(barcode_db_with_barcodes, test_function, column_name):
    barcodesInDB = barcode_db_with_barcodes[1]
