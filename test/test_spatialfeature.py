...
feature3 = spatialfeature.SpatialFeature(
    [[geometry.Polygon(testCoords1)], [geometry.Polygon(testCoords1)]],
    0,
    zCoordinates=np.array([0, 0.5]),
)
feature4 = spatialfeature.SpatialFeature(
    [[geometry.Polygon(testCoords1)], [geometry.Polygon(testCoords2)]],
    0,
    zCoordinates=np.array([0, 0.5]),
)
...
def test_feature_from_label_matrix():
    ...
    feature = spatialfeature.SpatialFeature.feature_from_label_matrix(testLabels, 0)
    ...
    assert feature.get_boundaries()[0][0].equals(
        geometry.Polygon(
            list(
                zip(
                    [2.1, 2.1, 2.0, 1.0, 0.9, 0.9, 1.0, 2.0, 2.1],
...
def test_feature_from_label_matrix_transform():
    testLabels = np.zeros((1, 4, 4))
    testLabels[0, 1:3, 1:3] = 1
    transformMatrix = np.array([[2.0, 0, 3], [0, 2.5, 1], [0, 0, 1]])
...
def test_feature_hdf5_db_read_write_delete_one_fov(single_task, simple_merfish_data):
    featureDB = spatialfeature.HDF5SpatialFeatureDB(simple_merfish_data, single_task)
...
def test_feature_hdf5_db_read_write_delete_multiple_fov(
    ...
    tempFeature2 = spatialfeature.SpatialFeature([[geometry.Polygon(testCoords2)]], 1)
    featureDB = spatialfeature.HDF5SpatialFeatureDB(simple_merfish_data, single_task)
...
def test_find_overlapping_cells():
    ...
    assert len(t2) == 0
    assert (
        (p3 in t3) and (p1 in t3) and (p4 in t3) and (p2 not in t3) and (p5 not in t3)
    )
    assert (
        (p4 in t4)
        and (p3 in t4)
...
def test_remove_overlapping_cells():
    ...
    G = spatialfeature.construct_graph(
        G, cells, spatialIndex, currentFOV, allFOVs, fovBoxes
    )
