...
def build_parser():
    ...
    parser.add_argument(
        "-o", "--data-organization", help="name of the data organization file to use"
    )
    parser.add_argument(
        "-c", "--codebook", nargs="+", help="name of the codebook to use"
    )
    parser.add_argument(
        "-m", "--microscope-parameters", help="name of the microscope parameters to use"
    )
    parser.add_argument("-p", "--positions", help="name of the position file to use")
    parser.add_argument(
        "-n", "--core-count", type=int, help="number of cores to use for the analysis"
    )
    parser.add_argument(
        "--check-done",
        action="store_true",
        help="flag to only check if the analysis task is " + "done",
    )
    parser.add_argument(
        "-t",
        "--analysis-task",
    ...
    parser.add_argument(
        "-k", "--snakemake-parameters", help="the name of the snakemake parameters file"
    )
    parser.add_argument(
        "--no_report",
...
def _get_input_path(prompt):
    while True:
        ...
        if not pathString.startswith("s3://") and not os.path.exists(
            os.path.expanduser(pathString)
        ):
            print(
                "Directory %s does not exist. Please enter a valid path." % pathString
            )
        else:
            return pathString
    ...
    if not args.generate_only:
        if args.analysis_task:
            ...
            if args.snakemake_parameters:
                with open(
                    os.sep.join(
                        [m.SNAKEMAKE_PARAMETERS_HOME, args.snakemake_parameters]
                    )
                ) as f:
    ...
    if report:
            ...
            with open(snakefilePath + ".stats") as f:
                requests.post(
                    ...
                    files={
                        "file": (
                            ".".join(
                                ["snakestats", dataSet.dataSetName, str(reportTime)]
                            )
                            + ".csv",
                            f,
                        )
        ...
        datasetMeta = {
            ...
            "sequential_count": len(
                dataSet.get_data_organization().get_sequential_rounds()
            ),
        ...
        with contextlib.suppress(requests.exceptions.RequestException):
            requests.post(
                ...
                files={
                    "file": (
                        ".".join([dataSet.dataSetName, str(reportTime)]) + ".json",
