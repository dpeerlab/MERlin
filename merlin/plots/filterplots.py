...
class CodingBarcodeSpatialDistribution(AbstractPlot):
    ...
    def get_required_tasks(self):
        return {"filter_task": "all", "global_align_task": "all"}
    ...
    def _generate_plot(self, inputTasks, inputMetadata):
        ...
        spatialMetadata = inputMetadata["filterplots/GlobalSpatialDistributionMetadata"]
        plt.imshow(
            spatialMetadata.spatialCodingCounts,
...
class BlankBarcodeSpatialDistribution(AbstractPlot):
    ...
    def _generate_plot(self, inputTasks, inputMetadata):
        ...
        spatialMetadata = inputMetadata["filterplots/GlobalSpatialDistributionMetadata"]
        plt.imshow(
            spatialMetadata.spatialBlankCounts,
            extent=spatialMetadata.get_spatial_extents(),
            cmap=plt.get_cmap("Greys"),
        )
        plt.xlabel("X position (pixels)")
...
class BarcodeRadialDensityPlot(AbstractPlot):
    ...
    def _generate_plot(self, inputTasks, inputMetadata):
        ...
        spatialMetadata = inputMetadata["filterplots/FOVSpatialDistributionMetadata"]
        singleColorCounts = spatialMetadata.singleColorCounts
        plt.plot(
            spatialMetadata.radialBins[:-1],
            singleColorCounts / np.sum(singleColorCounts),
        )
        multiColorCounts = spatialMetadata.multiColorCounts
        plt.plot(
            spatialMetadata.radialBins[:-1], multiColorCounts / np.sum(multiColorCounts)
        )
...
class FilteredBarcodeAbundancePlot(AbstractPlot):
    ...
    def _generate_plot(self, inputTasks, inputMetadata):
        ...
        codingDF = countDF[
            countDF.index.isin(codebook.get_coding_indexes())
        ].sort_values(by="counts", ascending=False)
        blankDF = countDF[countDF.index.isin(codebook.get_blank_indexes())].sort_values(
            by="counts", ascending=False
        )

        fig = plt.figure(figsize=(10, 5))
        plt.plot(np.arange(len(codingDF)), np.log10(codingDF["counts"]), "b.")
        plt.plot(
            np.arange(len(codingDF), len(countDF)), np.log10(blankDF["counts"]), "r."
        )
...
class AdaptiveFilterBarcodeDistributionPlots(AbstractPlot):
    ...
    def _generate_plot(self, inputTasks, inputMetadata):
        ...
        for i in range(min(len(areaBins), 6)):
            ...
            plt.imshow(
                blankHistogram[:, :, i].T + codingHistogram[:, :, i].T,
                extent=plotExtent,
                origin="lower",
                aspect="auto",
                cmap="OrRd",
...
class AdaptiveFilterMisidentificationVsAbundance(AbstractPlot):
    ...
    def _generate_plot(self, inputTasks, inputMetadata):
        ...
        selectMisidentification = filterTask.parameters["misidentification_rate"]
        selectThreshold = adaptiveTask.calculate_threshold_for_misidentification_rate(
...
class FOVSpatialDistributionMetadata(PlotMetadata):
    ...
    def _radial_distance(self, x: float, y: float) -> float:
        return np.sqrt((x - 0.5 * self._width) ** 2 + (y - 0.5 * self._height) ** 2)

    def _radial_distribution(self, inputBarcodes, barcodeIDs):
        selectBarcodes = inputBarcodes[inputBarcodes["barcode_id"].isin(barcodeIDs)]
        radialDistances = [
            self._radial_distance(r["x"], r["y"]) for i, r in selectBarcodes.iterrows()
        ]
    ...
    def _spatial_distribution(self, inputBarcodes, barcodeIDs):
        ...
        if len(selectBarcodes) > 1:
            return np.histogram2d(
                selectBarcodes["x"],
                selectBarcodes["y"],
                bins=(self.spatialXBins, self.spatialYBins),
            )[0]
        else:
            return 0
    ...
    def update(self) -> None:
        ...
        if updated:
            self._save_numpy_metadata(self.completeFragments, "complete_fragments")
            self._save_numpy_metadata(
                self.multiColorCounts, "multi_color_radial_counts"
            )
            self._save_numpy_metadata(
                self.singleColorCounts, "single_color_radial_counts"
            )
            self._save_numpy_metadata(self.spatialCodingCounts, "spatial_coding_counts")
            self._save_numpy_metadata(self.spatialBlankCounts, "spatial_blank_counts")
        ...
        for i in range(filterTask.fragment_count()):
            if not self.completeFragments[i] and filterTask.is_complete(i):
                ...
                self.barcodeCounts += np.histogram(
                    barcodes["barcode_id"], bins=np.arange(len(self.barcodeCounts) + 1)
                )[0]
...
class GlobalSpatialDistributionMetadata(PlotMetadata):
    ...
    def _spatial_distribution(self, inputBarcodes, barcodeIDs):
        ...
        if len(selectBarcodes) > 1:
            return np.histogram2d(
                selectBarcodes["global_x"],
                selectBarcodes["global_y"],
                bins=(self.spatialXBins, self.spatialYBins),
            )[0]
        else:
            return 0
