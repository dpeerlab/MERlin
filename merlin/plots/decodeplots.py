...
class AreaDistributionPlot(AbstractPlot):
    ...
    def _generate_plot(self, inputTasks, inputMetadata):
        ...
        fig = plt.figure(figsize=(4, 4))
        plt.bar(areaX, decodeMetadata.areaCounts, width=2 * shift)
...
class MeanIntensityDistributionPlot(AbstractPlot):
    ...
    def _generate_plot(self, inputTasks, inputMetadata):
        ...
        fig = plt.figure(figsize=(4, 4))
        plt.bar(intensityX, decodeMetadata.intensityCounts, width=2 * shift)
...
class AreaIntensityViolinPlot(AbstractPlot):
    ...
    def _generate_plot(self, inputTasks, inputMetadata):
        ...
        vpstats = [distribution_dict(x) for x in decodeMetadata.intensityCountsByArea]

        fig = plt.figure(figsize=(15, 5))
        ax = plt.subplot(1, 1, 1)
        ax.violin(
            vpstats,
            positions=decodeMetadata.areaBins[:-1],
            showmeans=True,
            showextrema=False,
        )

        if "filter_task" in inputTasks and isinstance(
            inputTasks["filter_task"], filterbarcodes.FilterBarcodes
        ):
            plt.axvline(
            ...
            plt.axhline(
                y=np.log10(inputTasks["filter_task"].parameters["intensity_threshold"]),
                color="green",
                linestyle=":",
            )
...
class DecodedBarcodesMetadata(PlotMetadata):
    def __init__(self, analysisTask, taskDict) -> None:
        ...
        if np.sum(self.completeFragments) >= min(20, self.decodeTask.fragment_count()):
    ...
    def _determine_bins(self):
        ...
        self.intensityCountsByArea = np.zeros(
            (len(self.areaBins) - 1, len(self.intensityBins) - 1)
        )
    ...
    def _extract_from_barcodes(self, barcodes):
        ...
        self.areaCounts += np.histogram(barcodes["area"], bins=self.areaBins)[0]
        self.intensityCounts += np.histogram(
            np.log10(barcodes["mean_intensity"]), bins=self.intensityBins
        )[0]
        self.distanceCounts += np.histogram(
            np.log10(barcodes["min_distance"]), bins=self.distanceBins
        )[0]

        for i, currentArea in enumerate(self.areaBins[:-1]):
            self.intensityCountsByArea[i, :] += np.histogram(
                np.log10(barcodes[barcodes["area"] == currentArea]["mean_intensity"]),
                bins=self.intensityBins,
            )[0]
    ...
    def update(self) -> None:
        ...
        if updated:
            ...
            self._save_numpy_metadata(self.intensityCountsByArea, "intensity_by_area")
