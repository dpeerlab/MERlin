...
def get_available_plots() -> set:
    ...
    for importer, modname, ispkg in pkgutil.iter_modules(merlin.plots.__path__):
        currentModule = importlib.import_module(merlin.plots.__name__ + "." + modname)
...
class PlotEngine:
    def __init__(self, plotTask, taskDict) -> None:
        ...
        requiredMetadata = {m for p in self.plotList for m in p.get_required_metadata()}
        self.metadataDict = {
            x.metadata_name(): x(plotTask, taskDict) for x in requiredMetadata
        }
    ...
    def take_step(self) -> bool:
        ...
        completeMetadata = [k for k, v in self.metadataDict.items() if v.is_complete()]
        readyPlots = [
            p for p in incompletePlots if p.is_ready(completeTasks, completeMetadata)
        ]
        for p in readyPlots:
