...
if os.path.exists(envPath):
    ...
    try:
        ...
        ANALYSIS_PARAMETERS_HOME = os.sep.join([PARAMETERS_HOME, "analysis"])
        CODEBOOK_HOME = os.sep.join([PARAMETERS_HOME, "codebooks"])
        DATA_ORGANIZATION_HOME = os.sep.join([PARAMETERS_HOME, "dataorganization"])
        POSITION_HOME = os.sep.join([PARAMETERS_HOME, "positions"])
        MICROSCOPE_PARAMETERS_HOME = os.sep.join([PARAMETERS_HOME, "microscope"])
        FPKM_HOME = os.sep.join([PARAMETERS_HOME, "fpkm"])
        SNAKEMAKE_PARAMETERS_HOME = os.sep.join([PARAMETERS_HOME, "snakemake"])

    except TypeError:
        print(
            "MERlin environment appears corrupt. Please run "
            + "'merlin --configure .' in order to configure the environment."
        )
else:
    print(
        (
            "Unable to find MERlin environment file at %s. Please run "
            + "'merlin --configure .' in order to configure the environment."
        )
        % envPath
    )
...
def get_analysis_datasets(maxDepth=2) -> List[dataset.DataSet]:
    ...
    for d in range(1, maxDepth + 1):
        metadataFiles += glob.glob(
            os.path.join(ANALYSIS_HOME, *["*"] * d, "dataset.json")
        )
