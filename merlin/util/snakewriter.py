...
class SnakemakeRule:
    ...
    def _expand_as_string(self, taskName, indexCount) -> str:
        return "expand(%s, g=list(range(%i)))" % (
            self._add_quotes(
    ...
    def _generate_current_task_inputs(self):
        inputTasks = [
            self._analysisTask.dataSet.load_analysis_task(x)
        ...
        if len(inputTasks) > 0:
            inputString = ",".join(
                [
                    "ancient("
                    + self._add_quotes(x.dataSet.analysis_done_filename(x))
                    + ")"
                    for x in inputTasks
                ]
            )
...
class SnakefileGenerator:
    ...
    def _parse_parameters(self):
        ...
        for tDict in self._analysisParameters["analysis_tasks"]:
            ...
            if newTask.get_analysis_name() in analysisTasks:
                raise Exception(
                    "Analysis tasks must have unique names. "
                    + newTask.get_analysis_name()
                    + " is redundant."
                )
    ...
    def generate_workflow(self) -> str:
        ...
        ruleList = {
            k: SnakemakeRule(v, self._pythonPath) for k, v in analysisTasks.items()
        }
