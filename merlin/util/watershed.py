...
def extract_seeds(seedImageStackIn: np.ndarray) -> np.ndarray:
    ...
    seedImages = ndimage.grey_erosion(
        seedImages, footprint=ndimage.morphology.generate_binary_structure(3, 1)
    )
    seedImages = np.array(
        [cv2.erode(x, diskStruct, borderType=cv2.BORDER_REFLECT) for x in seedImages]
    )
    ...
    seeds = np.array(
        [
            ndimage.morphology.binary_dilation(
                x, structure=morphology.diamond(28)[9:48, 9:48]
            )
            for x in seeds
        ]
    )
...
def separate_merged_seeds(seedsIn: np.ndarray) -> np.ndarray:
    ...
    for c in components:
        ...
        if all(x < 2 for x in seedCounts):
            ...
            seedPositions = np.round(
                [np.median([x.centroid for x in goodProperties], axis=0)]
            ).astype(int)
        else:
            goodFrames = [i for i, x in enumerate(seedCounts) if x > 1]
            goodProperties = [y for x in goodFrames for y in localProps[x]]
            ...
            km = kmedoids.kmedoids(
                goodCentroids,
                np.random.choice(
                    np.arange(len(goodCentroids)), size=np.max(seedCounts)
                ),
            )
            km.process()
            seedPositions = np.round(
                [goodCentroids[x] for x in km.get_medoids()]
            ).astype(int)
    ...
    seeds = np.array(
        [ndimage.morphology.binary_dilation(x, structure=diskStruct) for x in seeds]
    )
    ...
    watershedMask = np.array(
        [
            ndimage.morphology.binary_fill_holes(
                x
                > 1.1
                * filters.threshold_local(x, filterSize, method="mean", mode="nearest")
            )
