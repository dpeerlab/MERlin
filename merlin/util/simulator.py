...
class MERFISHDataFactory:
    ...
    def simulate_image(
        ...
        for p in spotPositions:
            upsampledImage[
                int(np.floor(p[0] * self.upsampleFactor)),
                int(np.floor(p[1] * self.upsampleFactor)),
            ] += 1000

        return self._downsample_image_stack([upsampledImage], addNoise=addNoise)[0]
    ...
    def simulate_dataset(
        ...
        for i in range(fovCount):
            ...
            for j in range(imageCount):
                fileName = (
                    "Conventional_750_650_561_488_405_"
                    + str(i)
                    + "_"
                    + str(j)
                    + ".tiff"
                ...
                imageData = np.zeros(shape=(5, *self.imageSize), dtype=np.uint16)
                firstBitIndex = [
                    i
                    for i, x in enumerate(self.bitOrganization)
                    if x[0] == j and x[1] == 0
                ][0]
                secondBitIndex = [
                    i
            ...
            np.save(
                os.sep.join([dataDir, "true_positions_" + str(i) + ".npy"]),
                rnaPositions,
            )
    ...
    def _simulate_fiducial_image(self):
        ...
        upsampledFiducials = (
            self.fiducialBrightness
            * np.histogram2d(
                fiducialPositions[:, 0] * self.imageSize[0],
                fiducialPositions[:, 1] * self.imageSize[1],
                bins=self.upsampleFactor * self.imageSize,
            )[0]
        )

        return self._downsample_image_stack([upsampledFiducials])[0]

    def _simulate_single_fov(self, codebook, barcodeAbundances, fluorophoreCount):
    ...
    def _add_spots_for_barcode(
        self, barcode, positions, fluorophoreCount, upsampledStack
    ):
        upsampledImage = np.zeros(self.upsampleFactor * self.imageSize)
        for p in positions:
            upsampledImage[
    ...
    def _downsample_image_stack(self, upsampledStack, addNoise=True):
        ...
        for i in range(len(imageStack)):
            blurredImage = cv2.GaussianBlur(
                upsampledStack[i].astype(float),
                ksize=(51, 51),
                sigmaX=self.upsampleFactor * self.psfSigma,
            )
            downsampledImage = np.array(
                PIL.Image.fromarray(
                    convolve2d(
                        blurredImage,
            ...
            if addNoise:
                imageStack[i] = np.random.poisson(downsampledImage + self.background)
            else:
