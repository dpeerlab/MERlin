...
class GenerateMosaic(analysistask.AnalysisTask):
    ...
    def get_dependencies(self):
        return [self.parameters["global_align_task"], self.parameters["warp_task"]]
    ...
    def _micron_to_mosaic_pixel(self, micronCoordinates, micronExtents) -> np.ndarray:
        """Calculates the mosaic coordinates in pixels from the specified
        global coordinates.
        """
        return np.matmul(
            self._micron_to_mosaic_transform(micronExtents),
            np.append(micronCoordinates, 1),
        ).astype(np.int32)[:2]

    def _micron_to_mosaic_transform(self, micronExtents: ExtentTuple) -> np.ndarray:
    ...
    def _transform_image_to_mosaic(
        ...
        return cv2.warpAffine(inputImage, transform[:2, :], mosaicDimensions)
    ...
    def _run_analysis(self):
        ...
        if "data_channels" in self.parameters:
            if isinstance(self.parameters["data_channels"], str):
                dataChannels = [
                    dataOrganization.get_data_channel_index(
        ...
        if not self.parameters["separate_files"]:
            ...
            with self.dataSet.writer_for_analysis_images(self, "mosaic") as outputTif:
                for d in dataChannels:
                    for z in zIndexes:
                        ...
                        outputTif.save(
                            mosaic, photometric="MINISBLACK", metadata=imageDescription
                        )
        else:
    ...
    def _prepare_mosaic_slice(
        self, zIndex, dataChannel, micronExtents, alignTask, maximumProjection
    ):
        ...
        if "optimize_task" in self.parameters:
            ...
            ).get_chromatic_corrector()

        cropWidth = self.parameters["fov_crop_width"]
        mosaicDimensions = tuple(
            self._micron_to_mosaic_pixel(micronExtents[-2:], micronExtents)
        )
        ...
        for f in self.dataSet.get_fovs():
            ...
            if self.parameters["draw_fov_labels"]:
                inputImage = cv2.putText(
                    inputImage,
                    str(f),
                    (int(0.2 * inputImage.shape[0]), int(0.2 * inputImage.shape[1])),
                    0,
                    10,
                    (65000, 65000, 65000),
                    20,
            ...
            transformedImage = self._transform_image_to_mosaic(
                inputImage, f, alignTask, micronExtents, mosaicDimensions
            )
