    ...
    def _run_analysis(self, fragmentIndex):
        ...
        if self.parameters["write_decoded_images"]:
            self._save_decoded_images(
                fragmentIndex, zPositionCount, decodedImages, magnitudeImages, distances
            )
    ...
    def _save_decoded_images(
        ...
        with self.dataSet.writer_for_analysis_images(self, "decoded", fov) as outputTif:
            for i in range(zPositionCount):
                outputTif.save(
                    decodedImages[i].astype(np.float32),
                    photometric="MINISBLACK",
                    metadata=imageDescription,
                )
                outputTif.save(
                    magnitudeImages[i].astype(np.float32),
                    photometric="MINISBLACK",
                    metadata=imageDescription,
                )
                outputTif.save(
                    distanceImages[i].astype(np.float32),
                    photometric="MINISBLACK",
                    metadata=imageDescription,
                )
