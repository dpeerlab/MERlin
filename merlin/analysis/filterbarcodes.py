...
class GenerateAdaptiveThreshold(analysistask.AnalysisTask):
    ...
    def calculate_threshold_for_misidentification_rate(
        self, targetMisidentificationRate: float
    ) -> float:
    ...
    def calculate_barcode_count_for_threshold(self, threshold: float) -> float:
        ...
        codingHistogram = self.get_coding_count_histogram()
        blankFraction = self.get_blank_fraction_histogram()
        return np.sum(blankHistogram[blankFraction < threshold]) + np.sum(
            codingHistogram[blankFraction < threshold]
        )
    ...
    def extract_barcodes_with_threshold(
        ...
        barcodeBins = (
            np.array(
                (
                    np.digitize(
                        selectData[:, 0], self.get_intensity_bins(), right=True
                    ),
    ...
    def _extract_counts(barcodes, intensityBins, distanceBins, areaBins):
        ...
        return np.histogramdd(
            barcodeData, bins=(intensityBins, distanceBins, areaBins)
        )[0]
    ...
    def _run_analysis(self):
        ...
        distanceBins = self.dataSet.load_numpy_analysis_result_if_available(
            "distance_bins",
            self,
            np.arange(0, decodeTask.parameters["distance_threshold"] + 0.02, 0.01),
        )
            ...
            if intensityBins is None or blankCounts is None or codingCounts is None:
                ...
                if np.sum(pendingFragments) >= min(20, self.fragment_count()):
                    ...
                    sampledFragments = np.random.choice(
                        [i for i, p in enumerate(pendingFragments) if p], size=20
                    )
                    intensityExtremes = [
                        extreme_values(
                            barcodeDB.get_barcodes(i, columnList=["mean_intensity"])[
                                "mean_intensity"
                            ]
                        )
...
class AdaptiveFilterBarcodes(AbstractFilterBarcodes):
    ...
    def _run_analysis(self, fragmentIndex):
        ...
        bcDatabase.write_barcodes(
            adaptiveTask.extract_barcodes_with_threshold(threshold, currentBarcodes),
            fov=fragmentIndex,
        )
