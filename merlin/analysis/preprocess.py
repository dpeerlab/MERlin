...
class Preprocess(analysistask.ParallelAnalysisTask):
    ...
    def get_pixel_histogram(self, fov=None):
        ...
        pixelHistogram = np.zeros(
            self.get_pixel_histogram(self.dataSet.get_fovs()[0]).shape
        )
...
class DeconvolutionPreprocess(Preprocess):
    def __init__(self, dataSet, parameters=None, analysisName=None) -> None:
        ...
        if "decon_filter_size" not in self.parameters:
            self.parameters["decon_filter_size"] = int(
                2 * np.ceil(2 * self.parameters["decon_sigma"]) + 1
            )
                ...
                [
                    [
                        self.get_processed_image(
                            fov,
                            self.dataSet.get_data_organization().get_data_channel_for_bit(
                                b
                            ),
                            zIndex,
                            chromaticCorrector,
    ...
    def _high_pass_filter(self, inputImage: np.ndarray) -> np.ndarray:
        ...
        hpImage = imagefilters.high_pass_filter(
            inputImage, highPassFilterSize, self._highPassSigma
        )
    ...
    def _run_analysis(self, fragmentIndex):
        ...
        for bi, b in enumerate(self.get_codebook().get_bit_names()):
            dataChannel = self.dataSet.get_data_organization().get_data_channel_for_bit(
                b
            )
            for i in range(len(self.dataSet.get_z_positions())):
                ...
                pixelHistogram[bi, :] += np.histogram(
                    deconvolvedImage, bins=histogramBins
                )[0]
