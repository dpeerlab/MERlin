...
class OptimizeIteration(decode.BarcodeSavingParallelAnalysisTask):
    ...
    def __init__(self, dataSet, parameters=None, analysisName=None):
        ...
        if "fov_index" in self.parameters:
            ...
            for i in range(self.parameters["fov_per_iteration"]):
                fovIndex = int(np.random.choice(list(self.dataSet.get_fovs())))
                zIndex = int(
                    np.random.choice(list(range(len(self.dataSet.get_z_positions()))))
                )
    ...
    def _run_analysis(self, fragmentIndex):
        ...
        di, pm, npt, d = decoder.decode_pixels(warpedImages, scaleFactors, backgrounds)
        ...
        self.dataSet.save_numpy_analysis_result(
            refactors, "scale_refactors", self.analysisName, resultIndex=fragmentIndex
        )
        self.dataSet.save_numpy_analysis_result(
            backgrounds,
            "background_refactors",
            self.analysisName,
            resultIndex=fragmentIndex,
        )
        self.dataSet.save_numpy_analysis_result(
    ...
    def _get_previous_chromatic_transformations(
        ...
        if "previous_iteration" not in self.parameters:
            ...
            return {
                u: {v: transform.SimilarityTransform() for v in usedColors if v >= u}
                for u in usedColors
            }
        else:
            previousIteration = self.dataSet.load_analysis_task(
    ...
    def _get_chromatic_transformations(
        ...
        if not self.is_complete():
            raise Exception(
                "Analysis is still running. Unable to get scale " + "factors."
            )
        ...
        # completely written
            ...
            colorPairDisplacements = {
                u: {v: [] for v in usedColors if v >= u} for u in usedColors
            }
            ...
            for fov in uniqueFOVs:
                ...
                for z in zIndexes:
                    ...
                    warpedImages = np.array(
                        [
                            warpTask.get_aligned_image(
                                fov,
                                dataOrganization.get_data_channel_for_bit(b),
                                int(z),
                                previousCorrector,
                            )
    ...
    def get_backgrounds(self) -> np.ndarray:
        if not self.is_complete():
            raise Exception(
                "Analysis is still running. Unable to get " + "backgrounds."
            )
        ...
        # completely written
            ...
            backgrounds = np.nanmedian(
                np.add(previousBackgrounds, np.multiply(refactors, previousFactors)),
                axis=0,
            )
