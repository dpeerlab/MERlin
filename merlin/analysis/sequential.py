...
class SumSignal(analysistask.ParallelAnalysisTask):
    ...
    def _extract_signal(self, cells, inputImage, zIndex) -> pd.DataFrame:
        cellCoords = []
        for cell in cells:
        ...
        propsOut = pd.DataFrame(
            data=[
                (propsDict[k].intensity_image.sum(), propsDict[k].filled_area)
                if k in propsDict
                else (0, 0)
    ...
    def _get_sum_signal(self, fov, channels, zIndex):
        ...
        for ch in channels:
            ...
            if self.highpass:
                ...
                img = imagefilters.high_pass_filter(
                    img, highPassFilterSize, highPassSigma
                )
        ...
        compiledSignal = pd.concat(signals, axis=1)
        compiledSignal.columns = channels + ["Pixels"]

        return compiledSignal

    def get_sum_signals(self, fov: int = None) -> pd.DataFrame:
        ...
        Returns:
        -------
            A DataFrame containing the sum signal information.
        """
        if fov is None:
            return pd.concat(
...
class ExportSumSignals(analysistask.AnalysisTask):
    ...
    def _run_analysis(self):
        ...
        self.dataSet.save_dataframe_to_csv(
            signals, "sequential_sum_signals", self.get_analysis_name()
        )
